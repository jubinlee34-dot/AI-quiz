...
같은 원본 자료에 초급/중급/심화 프롬프트를 적용했을 때 결과가 어떻게 달라지는지 비교하는 학습용 퀴즈 앱입니다.
프롬프트의 빈칸을 드래그 앤 드롭으로 채우면 결과를 확인할 수 있습니다.

<details>
<summary><strong>▶ 퀴즈 실행하기</strong></summary>

<br>

👉 [AI 프롬프트 퀴즈 바로 실행](https://jubinlee34-dot.github.io/AI-quiz/)
...
- **심화**: 조건을 단순히 늘리는 단계가 아니라 비교·판단·불확실성 처리·자체검수를 포함하는 단계. 프롬프트를 모두 완성해야 결과가 공개됩니다.

현재 문제팩: **v1 (초급 7문제 + 중급 7문제 + 심화 7문제 = 총 21문제)**

---

## 로컬 실행 방법

로컬에서 직접 실행할 경우 서버로 실행해야 합니다. `index.html`을 더블클릭해서 `file://`로 열면
...
| 항목 | 설명 |
|---|---|
| `difficulty` | `"초급"`, `"중급"` 또는 `"심화"`. 결과 잠금 여부가 여기서 결정됩니다. |
| `sourceId` | 같은 원본을 쓰는 초급/중급/심화 문제를 묶는 키. 같은 원본의 문제는 같은 값을 씁니다. |
| `order` | 문제 표시 순서. |
